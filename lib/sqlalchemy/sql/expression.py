...
class _BindParamClause(ClauseElement, _CompareMixin):
    ...
    def __init__(self, key, value, type_=None, unique=False, isoutparam=False, shortname=None):
        ...
        if unique:
            self.key = "ANON_%d_%s" % (id(self), key or 'param')
        else:
            self.key = key or "ANON_%d_param" % id(self)
    ...
    def _clone(self):
        ...
        if self.unique:
            c.key = "ANON_%d_%s" % (id(c), c._orig_key or 'param')
        return c

    def _convert_to_unique(self):
        if not self.unique:
            self.unique=True
            self.key = "ANON_%d_%s" % (id(self), self._orig_key or 'param')
...
class Alias(FromClause):
    ...
    def __init__(self, selectable, alias=None):
        ...
        if alias is None:
            ...
            alias = 'ANON_%d_%s' % (id(self), alias or 'anon')
        self.name = alias
...
class _Label(ColumnElement):
    ...
    def __init__(self, name, obj, type_=None):
        while isinstance(obj, _Label):
            obj = obj.obj
        self.name = name or "ANON_%d_%s" % (id(self), getattr(obj, 'name', 'anon'))
