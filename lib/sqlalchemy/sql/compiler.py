...
class DefaultCompiler(engine.Compiled):
    ...
    def visit_column(self, column, result_map=None, **kwargs):
        ...
        if column._is_oid:
            ...
            name = self.preparer.truncated_identifier("colident", column.name)
        else:
            name = column.name
    ...
    def _truncate_bindparam(self, bindparam):
        ...
        bind_name = self.preparer.truncated_identifier("bindparam", bind_name)
...
#            correlate_froms = correlate_froms.union(existingfroms)
    ...
    def visit_table(self, table, asfrom=False, **kwargs):
        if asfrom:
            name = self.preparer.truncated_identifier("tblident", table.name)
            if getattr(table, "schema", None):
                return self.preparer.quote(table, table.schema) + "." + self.preparer.quote(table, name)
            else:
...
class IdentifierPreparer(object):
    ...
    def format_sequence(self, sequence, use_schema=True):
        name = self.quote(sequence, self.truncated_identifier("tblident", sequence.name))
    ...
    def format_constraint(self, constraint):
        return self.quote(constraint, self.truncated_identifier("tblident", constraint.name))

    def format_index(self, index):
        return self.quote(index, self.truncated_identifier("tblident", index.name))
    ...
    def format_table(self, table, use_schema=True, name=None):
        ...
        if name is None:
            name = table.name
        name = self.truncated_identifier("tblident", name)
    ...
    def format_column(self, column, use_table=False, name=None, table_name=None):
        ...
        if name is None:
            name = column.name
        name = self.truncated_identifier("colident", name)
